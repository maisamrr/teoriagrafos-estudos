    ...
    while(aux =! NULL) {
        ...
        if(aux->valor != pai) {
            ...
            if(vertices[aux->valor].visitado == 1) {
                // foi visitado, achou back edge
                printf("\nback edge");
                printf("\nAresta %d com %d e backedge", x, aux->valor);
                // atualiza o low do atual para o menor entre o low do atual e o in do ancestral dele
                vertices[x].low = min(vertices[x].low, vertices[aux->valor].in);
            } else {
                // não foi visitado, chama dfs
                dfs(vertices, aux->valor, x);
                // verifica se é ponte
                if(vertices[x].in < vertices[aux->valor].low) {
                    printf("\nponte, %d ---- %d", x, aux->valor);
                }
                // atualiza o low do atual para o menor entre o low dele ou do ancestral
                vertices[x].low = min(vertices[x].low, vertices[aux->valor].low);
            }
        }
        aux = aux->prox;
    }
